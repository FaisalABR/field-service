pipeline {
  ...
  environment {
    IMAGE_NAME = 'field-service' // diganti dengan nama image Docker yang akan dibuat
  ...
  stages {
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/FaisalABR/field-service.git
    ...
    stage('Deploy to Remote Server') {
      steps {
        script {
          def targetDir = "home/faisal/belajar-web/golang/soccer-microservices/field-service" // diganti dengan direktori tempat menyimpan kode di server
          ...
          ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
            if [ -d "${targetDir}/.git" ]; then
                ...
                git clone -b "${TARGET_BRANCH}" git@github.com:FaisalABR/field-service.git "${targetDir}" 
                cd "${targetDir}"
            fi
